...
/**
 * REST controller for managing {@link com.manthan.bank.domain.Customer}.
 */
@RestController
@RequestMapping("/api")
public class CustomerResource {
     ...
     *         the updated customerDTO, or with status {@code 400 (Bad Request)} if
     *         the customerDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the customerDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<List<CustomerDTO>> getAllCustomers(Pageable pageable,
        ...
        if (queryParams.get("defaulter").indexOf("true") != -1) {
            page = customerService.findDefaulters(pageable, true);
        } else{
