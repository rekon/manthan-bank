...
export const getEntities: ICrudGetAllAction<ICustomer> = (page, size, sort, showDefaulters) => {
  const requestUrl = `${apiUrl}${sort ? `?page=${page}&size=${size}&sort=${sort}&defaulter=${showDefaulters}` : ''}`;
  return {
